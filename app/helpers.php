...
function utf8_clean_string($text)
{
    ...
    if (empty($homographs))
    {
        $homographs = Config::get('utf\data\confusables');
    }
...
function utf8_case_fold($text, $option = 'full')
{
    ...
    if (!isset($uniarray['c']))
    {
        //$uniarray['c'] = config('case_fold_c');
        $uniarray['c'] = Config::get('utf\data\case_fold_c');
    }

    // only set full if we need to
    if ($option === 'full' && !isset($uniarray['f']))
    {
        $uniarray['f'] = config('utf\data\case_fold_f');
    }

    // only set simple if we need to
    if ($option !== 'full' && !isset($uniarray['s']))
    {
        $uniarray['s'] = config('utf\data\case_fold_s');
    }
